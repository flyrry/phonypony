...
dumbBot state = let board = gameBoard $ stateGame state
                    ...
                    -- position on a map if it is possible to reach it
                    boardMap = buildBoardMap board hero
                    -- collect all goals on the map like taverns, mines
                    -- which do not belong to us and all enemies
                    goals = getGoals state
                    -- assign a value to each goal which indicates what
                    -- a potential profit (can be negative) it might give us
                    ...
                    -- sort goals first by the score (potential profit) and
                    -- if it is the same (e.g. every mine should potentially
                    -- give the same amount of profit) then we select the
                    -- closest one to our hero
                    ...
                    -- will be unreachable then we simply Stay in our place
                    -- TODO: maybe try taking all goals which are profitable
